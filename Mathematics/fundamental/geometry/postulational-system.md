---
description: Fundamental postulates, definitions, and theorems of Euclidean geometry.
---
...
Lines are one of a few one-dimensional figures. Lines are defined by two points and go infinitely in both directions. Rays are defined by two points and go infinitely in only one direction; a ray’s endpoint is the point where a rays stops. Line segments are defined by two points and always end at both points.
...
The most exciting part about figures, however, is when they intersect.

## Angles

When lines, rays, and line segments intersect, they create angles. Angles can have measures from 0 to 360 degrees.

### Angle Names

Angles are named based on how large or small their measures are. The following table represents all angle names; all angle ranges are inclusive...

| Angle Measure Range | Name of Angle  |
| ------------------: | -------------- |
|              1°-89° | Acute Angle    |
|                 90° | Right Angle    |
|            91°-179° | Obtuse Angle   |
...
Knowing the angle names is crucial for understanding how large its measure is.

### Angle Relationships

The following are all the ways angles relate to other angles.
...
A statement is a theorem if and only if it is true in _every_ case.

A relation is an association between two elements in a space. A relation is an equivalence relation if and only if it satisfies the reflexive, symmetric, and transitive properties.
...
Keep in mind that though these are properties, they are only all satisfied only by the relation of congruence. Thus, congruence is an equivalence relation.
